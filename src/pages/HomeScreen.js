...
const HomeScreen = () => {
  ...
  useEffect(() => {
    ...
    //Verifica se user não existe ou é null, se for, setLoading(false) é chamado para indicar que o carregamento está completo, e a função retorna, encerrando a execução do efeito.
    if(!user) {
      setLoading(false);
      return;
    };

    //Esta é uma função assíncrona chamada handleGetInformations. Ela aguarda a obtenção de informações do usuário e, em seguida, chama setLoading(false) para indicar que o carregamento está completo.
    async function handleGetInformations() {
      await getUserInformations(user.uid);
      setLoading(false);
    }
    // A função handleGetInformations é chamada dentro do useEffect para buscar as informações do usuário.
    handleGetInformations();
  }, [])
//Se loading for verdadeiro, o componente retorna um indicador de carregamento 
